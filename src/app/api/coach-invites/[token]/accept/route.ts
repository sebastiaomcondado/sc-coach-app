...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  await admin.from("profiles").update({ team_owner_id: invite.team_owner_id }).eq("id", coachId);

  await admin.from("coach_invites").update({ used_at: new Date().toISOString() }).eq("token", token);
